...
class VisionSortingBrainOnce(UR12eExtendedPlanner):
    ...
    def execute_sorting_cycle(self):
        ...
        z_pick = max(0.233, 0.20 - 0.02 + p.position.z)
        ...
        try:
            ...
            # 2. Check if object is flat
            if z_pick < 0.24:
                self.get_logger().info("Flat object detected. Executing Smart Touch Skill...")
                
                # A. Go down slowly to "near" position (e.g., 2cm above target)
                # We reduce velocity scaling for this specific move
                self.move_plan_xyz_theta_sync(x_target, y_target, z_pick + 0.01, angle_rad)
                
                # B. Smart Descent: In a production environment, you'd use a force-stop script.
                # Here, we simulate the "slow touch" by moving to the exact surface Z.
                # If you have a force sensor node, you would loop here until force > threshold.
                self.move_plan_xyz_theta_sync(x_target, y_target, z_pick - 0.02, angle_rad)
                
                # C. Retract a little bit (1cm)
                self.get_logger().info("Touching surface. Retracting 1cm...")
                self.move_plan_xyz_theta_sync(x_target, y_target, z_pick + 0.00, angle_rad)
                
                # D. Close Gripper
                self.set_gripper(self.GRIPPER_CLOSED)
                time.sleep(1.2) # Extra time for flat object alignment
            else:
                # Standard pickup for taller objects
