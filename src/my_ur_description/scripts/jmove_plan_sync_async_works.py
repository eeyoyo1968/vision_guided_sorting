...
def main(args=None):
    ...
    def wait_with_spin(node, duration):
        """Replacement for time.sleep that keeps ROS 2 communication alive."""
        start = node.get_clock().now()
        while (node.get_clock().now() - start).nanoseconds < duration * 1e9:
            rclpy.spin_once(node, timeout_sec=0.1)

    # 1. Test jmove_plan_async
    ...
    wait_with_spin(planner, 8) # Keeps communication alive during move
    ...
    wait_with_spin(planner, 7) # Fixed: allows trajectory to stream

    # 3. Final Moves (Sync version handles spinning internally)
