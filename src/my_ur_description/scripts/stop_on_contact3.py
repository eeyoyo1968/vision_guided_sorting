...
class URScriptForceTester(Node):
    ...
    def run_test(self):
        ...
        script = (
            ...
            "  \n"
            "  # stop_on_contact is a built-in UR function \n"
            "  # but for a manual I/O test, we move while checking the input\n"
            "  \n"
            "  # Start the move (No keyword 'wait' needed)\n"
            "  movel(p_target, a=0.2, v=0.02)\n"
            "  \n"
            "  # While moving, if DI_0 is false, we keep 'syncing'\n"
            "  # If DI_0 becomes true, we break the loop and stop\n"
            "  while (get_digital_in(0) == False):\n"
            "    # We need to check if we've reached the target position to stop the loop\n"
            "    if (point_dist(get_actual_tcp_pose(), p_target) < 0.001):\n"
            "      break\n"
            "    end\n"
            "    sync()\n"
            "  end\n"
            "  \n"
            "  stopl(2.0)\n"
            "  textmsg(\"CONTACT DETECTED OR TARGET REACHED\")\n"
            "  \n"
            "  # Small retract\n"
