...
class VisionSortingBrainOnce(UR12eExtendedPlanner):
    ...
    def retract_relative_sync(self, distance_m=0.004, speed=0.05):
        """Lifts the tool relative to its CURRENT position using URScript."""
        self.get_logger().info(f"Retracting {distance_m*1000}mm from current contact point...")
    
        script = (
            "def dynamic_retract():\n"
            "  # Capture current actual pose [x, y, z, rx, ry, rz]\n"
            "  current_p = get_actual_tcp_pose()\n"
            "  # Create target pose by adding distance to the Z-axis\n"
            "  target_p = pose_add(current_p, p[0, 0, " + str(distance_m) + ", 0, 0, 0])\n"
            "  # Move to the new Z height\n"
            "  movel(target_p, a=1.2, v=" + str(speed) + ")\n"
            "end\n"
            "dynamic_retract()\n"
        )
        ...
        # Briefly wait for the short move to complete
        time.sleep(0.5)
    ...
    def execute_sorting_cycle(self):
        ...
        try:
            ...
            if z_pick < 0.528:
                ...
                # Retract and Grip
                # 2. DYNAMIC RETRACT: Lift 4mm from wherever it actually stopped
                #self.retract_relative_sync(distance_m=0.025, speed=0.02)
                #time.sleep(1.0)
                #self.move_plan_xyz_theta_sync(x_target, y_target, z_pick + 0.0001, angle_rad, speed=0.02)
                #time.sleep(1.0)
                #self.set_gripper(self.GRIPPER_CLOSED)
                #time.sleep(1.0)


                # 1. Perform smart descent
                self.smart_descent_skill_xyz_theta(x_target, y_target, floor_z, angle_rad, duration=3.0)

                # 2. Wait a split second for the robot to settle so the Pose is accurate
                time.sleep(0.2)

                # 3. Dynamic Retract in Python
                self.move_plan_xyz_theta_sync(x_target, y_target, z_pick + 0.0008, angle_rad, speed=0.02)
                #self.retract_relative_python(distance_m=0.025, speed=0.02)
                #self.retract_relative_sync(distance_m=0.025, speed=0.02)
                #self.smart_retract_python(distance_m=0.004, speed=0.01)
            ...
            self.move_plan_xyz_theta_async(0.0, 0.6, 0.5, 0.0, speed=0.1)
            return True

        except Exception as e:
            self.get_logger().error(f"Cycle failed: {e}")
            # If it fails, we go home and WAIT for a new vision detection
            self.jmove_plan_sync(self.home_seed)
            return False
        finally:
            self.is_busy = False
